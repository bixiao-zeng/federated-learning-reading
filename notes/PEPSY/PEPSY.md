...
## Paper Original Framework

### Figure 2(a): Overall Workflow

![PEPSY overall server-client workflow](assets/figure-2a-overall-workflow.png)

论文原图 Figure 2(a)：PEPSY 包含客户端训练和服务端聚合两个阶段。普通神经组件使用 FedAvg 聚合；客户端的 data-missing profiles 使用 probabilistic synchronization 对齐并聚合为全局 profile。

### Figure 2(b): Client Design

![PEPSY client design](assets/figure-2b-client-design.png)

论文原图 Figure 2(b)：客户端先提取 modality-specific 和 data-specific features，再查询 data-missing profile 得到 missing-pattern features，最后将三类信息组合为 data-complete features，用于模态融合和下游预测。

> Source: Figure 2 in *Learning Reconfigurable Representations for Multimodal Federated Learning with Missing Data*, NeurIPS 2025. Images are extracted directly from the paper PDF without redrawing.

### 完整实现流程与 Tensor Shapes

![PEPSY complete implementation workflow with missing simulation and tensor shapes](assets/pepsy-complete-implementation-workflow.png)

该图结合论文方法与公开代码实现，补充展示了 `p_m` / `p_s` 缺失模拟、缺失值表示、重配置特征构造及关键 tensor shapes。需要特别注意：

- `p_m` 控制候选缺失模态比例，`p_s` 控制这些模态中的缺失样本比例，整体 missing degree 为 `p_m × p_s`。
- 论文用 missing matrix 中的 `0` 表示缺失；公开代码将缺失原始信号整段置为 `-1`，再据此生成 `missing_mask`。
- 缺失模态的编码特征先被临时置为零向量，随后使用同一样本可用模态编码特征的均值 `mean_shared` 替换；这里不涉及 image-to-image 生成。
- Top-k controls 的均值形成 `w_mis`，并与 `w_mod`、`w_ins` 拼接得到重配置特征 `w ∈ R^{B×M×3H}`。
