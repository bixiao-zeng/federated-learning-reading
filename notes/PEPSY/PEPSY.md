...
```mermaid
flowchart LR
    A["真实 MMFL 数据<br/>客户端模态集合不同<br/>模态内部特征也可能缺失"]
    B["关键问题<br/>本地数据视角不同<br/>导致表示空间错位"]
    C["研究缺口<br/>已有方法通常只处理<br/>一种缺失事件"]
    D["核心假设<br/>缺失模式可编码为<br/>可学习的 data-missing profile"]
    E["PEPSY<br/>用 embedding controls<br/>重配置共享表示"]
    F["联邦协作<br/>普通参数使用 FedAvg<br/>controls 使用非参数匹配聚合"]
    G["理论验证<br/>输出偏差受 L_ds、<br/>缺失模态数和模型平滑性控制"]
    H["实验验证<br/>主实验、分布偏移、消融、<br/>表示对齐与 profile 收敛"]
    I["结论<br/>严重缺失和 Non-IID 下<br/>获得更稳定的性能"]
...
```mermaid
flowchart TB
    S0["服务端全局模型 θ 与全局 control pool Ψ"]

    subgraph C["客户端 k：基于本地不完整多模态数据训练"]
        C1["本地数据 D_k<br/>客户端间缺模态 + 样本内缺特征"]
        C2["提取 modality-specific 表示 w_mod<br/>和 data-specific 表示 w_ins"]
        C3["构造 query<br/>并与 control pool Ψ_k 匹配"]
        C4["选择 top-k controls<br/>聚合得到 missing-pattern 表示 w_mis"]
        C5["拼接并重配置表示<br/>w = [w_mod; w_ins; w_mis]"]
        C6["跨模态对齐、重建与融合"]
        C7["任务预测"]
        C8["本地目标<br/>L_task + λ(L_ds + L_rc) - ηR"]
        C9["反向传播<br/>更新模型参数与被选中的 controls"]

        C1 --> C2 --> C3 --> C4 --> C5 --> C6 --> C7 --> C8 --> C9
    end

    subgraph S["服务端：聚合客户端知识"]
        S1["接收各客户端本地模型与 control pool"]
        S2["普通模型参数<br/>按 FedAvg 聚合"]
        S3["相似 controls<br/>非参数匹配与概率同步"]
        S4["形成新的全局模型 θ<br/>与代表性全局 control pool Ψ"]
...
图中需要区分两个尺度：客户端内部通过 top-k 查询选择与当前 batch/样本缺失上下文相关的 controls，并通过本地损失更新它们；服务端则跨客户端匹配语义相似的 controls，形成下一轮共享的全局 pool。论文算法强调 profile 可变大小与非参数聚合；公开代码实现中，各参与客户端每轮从同一个全局 pool 开始训练，动态大小主要由服务端聚合结果决定。
